...
namespace Prime31
{
	public class StoreKitGUIManager : MonoBehaviourGUI
	{
		...
		void OnGUI()
		{
			...
			if( toggleButtonState( "Show Deferred Payment Buttons" ) )
				coreFeatureButtonsGUI();
			else
				deferredPaymentsGUI();
			toggleButton( "Show Deferred Payment Buttons", "Show Core Buttons" );
			...
			if( GUILayout.Button( "Display Ask For Review" ) )
			{
				StoreKitBinding.askForReview();
			}
		...
		void coreFeatureButtonsGUI()
		{
			...
			if( _products != null && _products.Count > 0 )
			{
				GUILayout.Label( "Store Promotion Visibility" );
				if( GUILayout.Button( "Fetch Store Promotion Visibility" ) )
				{
					var product = _products[Random.Range( 0, _products.Count )];

					Debug.Log( "preparing to fetch promotion visibility for product: " + product.productIdentifier );
					StoreKitBinding.fetchStorePromotionVisibilityForProduct( product.productIdentifier );
				}


				if( GUILayout.Button( "Set Store Promotion Visibility" ) )
				{
					var product = _products[Random.Range( 0, _products.Count )];

					Debug.Log( "preparing to fetch promotion visibility for product: " + product.productIdentifier );
					StoreKitBinding.updateStorePromotionVisibilityForProduct( product.productIdentifier, SKProductStorePromotionVisibility.Hide );
				}
			}
		}


		void deferredPaymentsGUI()
		{
			if( GUILayout.Button( "Set Should Defer Store Payments" ) )
			{
				StoreKitBinding.setShouldDeferStorePayment( true );
			}


			if( GUILayout.Button( "Is Deferred Payment Cached" ) )
			{
				Debug.Log( "is there a deferred payment? " + StoreKitBinding.isDeferredPaymentCached() );
			}


			if( GUILayout.Button( "Get Deferred Payment Product ID" ) )
			{
				Debug.Log( "deferred payment product ID: " + StoreKitBinding.getDeferredPaymentProductIdentifier() );
			}
		}
